...
describe('Loader', function () {
  ...
  fs.readdirSync(samplesDir).forEach(function (jsFile) {
    ...
    var yamlFile = path.resolve(
      samplesDir,
      path.basename(jsFile, '.js') + '.yml',
    );

    it(path.basename(jsFile, '.js'), async function () {
      var expected = await import(path.resolve(samplesDir, jsFile)).then(
        (m) => m.default,
      );
      var actual = [];

      yaml.loadAll(
        fs.readFileSync(yamlFile, { encoding: 'utf8' }),
        function (doc) {
          actual.push(doc);
        },
        {
