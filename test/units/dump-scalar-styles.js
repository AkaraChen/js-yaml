...
describe('Scalar style dump:', function () {
  describe('Plain style', function () {
    ...
    it('disallows flow indicators inside flow collections', function () {
      assert.strictEqual(
        yaml.dump({ quote: 'mispell [sic]' }, { flowLevel: 0 }),
        "{quote: 'mispell [sic]'}\n",
      );
      assert.strictEqual(
        yaml.dump({ key: 'no commas, either' }, { flowLevel: 0 }),
