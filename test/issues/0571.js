...
describe('Undefined', function () {
  ...
  it('Should replace undefined with null in collections', function () {
    ...
    assert(str.match(/^\[/));
    assert.deepStrictEqual(yaml.load(str), [null, 1, null, null, 2]);
  ...
  it('Should remove keys with undefined in mappings', function () {
    ...
    str = yaml.dump(
      { t: undefined, foo: 1, bar: undefined, baz: null },
      { flowLevel: 0 },
    );
    assert(str.match(/^\{/));
    assert.deepStrictEqual(yaml.load(str), { foo: 1, baz: null });
  ...
  it('Should serialize undefined if schema is available', function () {
    assert.deepStrictEqual(
      yaml.load(yaml.dump([1, undefined, null, 2], { schema: undef_schema }), {
        schema: undef_schema,
      }),
      [1, undefined, null, 2],
    );
